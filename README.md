# 262P Milestone 5
Code modifications are in XML.java from line#1368 to the line#1400 also line#55, and XMLTest.java from line#1399 to line#1473

logic:

public static Future<Boolean> toJSONObject(Reader, Consumer<JSONObject>, Consumer<Exception>)

* Used Future api to make toJSONObject function return future task
* In XML class creates a Threadpool with fix 4 threads
* When user wants to use it, user can call this toJSONObject method return future.
* If future isn't done user can make while loop to do some other thing first.
...
Logic:
Create node class to get each key from JSONObject, then use toStream method to stream JSONObject. In toStream method, create new node class return Stream node of JSONObject
...
* Start at line#1117
* The parseModified() takes an additional Function<String, String> keyTransformer parameter, and applies the function at line#1234 and line#1304 to transform keys in XML files.
...
* Start at line#1326
* Take an additional Function<String, String> keyTransformer parameter
* Call the parseModified() method
...
Code modifications are in XML.java from line#875 to the line#1112 (task2 starts at line#875, task5 starts at line#1021). Unit Tests are in XMLTest.java from line#1075 to the line#1302 (total 7 test cases, 4 for task2, 3 for task5).
...
Logic :
* If found key, start append parsing and stop at key close bracket
* If key path value is a JsonArray, then keep append parsing until the end of JsonArray
